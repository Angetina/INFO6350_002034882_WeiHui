...
class _GameListScreenState extends State<GameListScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          Expanded(child: _buildActiveGamesList()),
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (docs == null || docs.isEmpty) {
          return const Center(child: Text('No active or waiting games found.'));
        }
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          ...
          if (snapshot.error.toString().toLowerCase().contains('index')) {
            errorText +=
                '\nPlease create the required Firestore index (status Asc, endedAt Desc) in Firebase Console.';
          }
          return Padding(
  ...
  Widget _buildActiveGameTile(BuildContext context, DocumentSnapshot gameDoc) {
    ...
    final playerXName = playerXData['displayName'] ?? "Unknown Player";
    final playerOName = playerOData?['displayName'] ?? "Waiting...";

    final status = data['status'] ?? "Unknown";
    final displayStatus = status[0].toUpperCase() + status.substring(1);
    final createdAtData = data['createdAt'];
    final started = _formatTimestamp(createdAtData);
  ...
  Widget _buildCompletedGameTile(
    ...
    final playerXName = playerXData['displayName'] ?? "Player X";
    final playerOName = playerOData['displayName'] ?? "Player O";

    final winner = data['winner'];
    final endedAtData = data['endedAt'];
    final endedDate = _formatTimestamp(endedAtData);
  ...
  void _createNewGame(BuildContext context) async {
    ...
    if (user == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('請先登入再建立新遊戲')));
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      Navigator.pop(context); // 關掉 loading
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('建立遊戲失敗：$e')));
    }
  }
}
